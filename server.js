...
async function initDb() {
  await pool.query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_comments_thread_created_at ON comments(thread, created_at DESC);
...
app.get('/comments', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, thread, author, content, at_seconds, created_at
       FROM comments
       WHERE thread = $1
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO comments (thread, author, content, at_seconds)
       VALUES ($1, $2, $3, $4)
       RETURNING id, thread, author, content, at_seconds, created_at`,
